...
async function scrapeG1Especiais() {
  try {
    const html = await fetchWithRetry("https://g1.globo.com/especiais/");
    ...
    console.error("Erro G1 Especiais:", error.message);
    return [];
  }
}
...
async function updateCurriculo() {
  try {
    console.log("🔄 Fazendo scraping de currículo...");
    const [g1] = await Promise.allSettled([scrapeG1Especiais()]);
    ...
    if (allArticles.length === 0) {
      console.log("⚠️ Nenhum conteúdo de currículo encontrado");
      return;
    }
    console.log(
    ...
    const filePath = path.join(__dirname, "../src/data/json/curriculo.json");
    let currentData = [];
    try {
    ...
    if (newArticles.length === 0) {
      console.log("ℹ️ Nenhum conteúdo novo de currículo encontrado");
      return;
    }
    currentData = [...newArticles, ...currentData].slice(0, 50);
    await fs.writeFile(filePath, JSON.stringify(currentData, null, 2));
    console.log(
      `✅ ${newArticles.length} novos conteúdos de currículo adicionados`
    );
    console.log(`📊 Total de currículo: ${currentData.length}`);
  } catch (error) {
