...
async function scrapeG1Educacao() {
  try {
    const html = await fetchWithRetry("https://g1.globo.com/educacao/");
    ...
    console.error("Erro G1 Educação:", error.message);
    return [];
  }
}
...
async function updateEducacao() {
  try {
    console.log("🔄 Fazendo scraping de educação...");
    const [g1] = await Promise.allSettled([scrapeG1Educacao()]);
    ...
    console.log(
      `   G1 Educação: ${
        g1.status === "fulfilled"
          ? "✅ OK (" + g1.value.length + " artigos)"
          : "❌ FALHOU - " + g1.reason?.message
      }`
    );

    const allArticles = [...(g1.status === "fulfilled" ? g1.value : [])];
    if (allArticles.length === 0) {
      console.log("⚠️ Nenhuma notícia de educação encontrada");
      return;
    }
    console.log(
    ...
    const filePath = path.join(
      __dirname,
      "../src/data/json/educacao-carreira.json"
    );
    let currentData = [];
    try {
    ...
    if (newArticles.length === 0) {
      console.log("ℹ️ Nenhuma notícia nova de educação encontrada");
      return;
    }
    currentData = [...newArticles, ...currentData].slice(0, 50);
    await fs.writeFile(filePath, JSON.stringify(currentData, null, 2));
    console.log(
      `✅ ${newArticles.length} novas notícias de educação adicionadas`
    );
    console.log(`📊 Total de educação: ${currentData.length}`);
  } catch (error) {
    console.error("❌ Erro ao atualizar educação:", error.message);
    process.exit(1);
  }
}
