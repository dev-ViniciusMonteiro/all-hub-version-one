...
async function scrapeG1Cultura() {
  try {
    const html = await fetchWithRetry("https://g1.globo.com/cultura/");
    ...
    console.error("Erro G1 Cultura:", error.message);
    return [];
  }
}

async function scrapeUOLSplash() {
  try {
    const html = await fetchWithRetry("https://www.uol.com.br/splash/");
    const $ = cheerio.load(html);
    const articles = [];
    $("article, .news-item")
      .slice(0, 2)
    ...
    console.error("Erro UOL Splash:", error.message);
    return [];
  }
}

async function scrapeTerra() {
  try {
    const html = await fetchWithRetry("https://www.terra.com.br/cultura/");
    const $ = cheerio.load(html);
    const articles = [];
    $("article, .news-item")
      .slice(0, 2)
    ...
    console.error("Erro Terra:", error.message);
    return [];
  }
}
...
async function updateEntretenimento() {
  try {
    console.log("🔄 Fazendo scraping de entretenimento...");
    const [g1, uol, terra] = await Promise.allSettled([
      scrapeG1Cultura(),
      scrapeUOLSplash(),
      scrapeTerra(),
    ]);
    ...
    if (allArticles.length === 0) {
      console.log("⚠️ Nenhuma notícia de entretenimento encontrada");
      return;
    }
    console.log(
    ...
    const filePath = path.join(
      __dirname,
      "../src/data/json/entretenimento.json"
    );
    let currentData = [];
    try {
    ...
    const existingTitles = new Set(
      currentData.map((item) => item.title.toLowerCase())
    );
    const newArticles = formattedArticles
      .filter((article) => !existingTitles.has(article.title.toLowerCase()))
      .slice(0, 6);
    if (newArticles.length === 0) {
      console.log("ℹ️ Nenhuma notícia nova de entretenimento encontrada");
      return;
    }
    currentData = [...newArticles, ...currentData].slice(0, 50);
    await fs.writeFile(filePath, JSON.stringify(currentData, null, 2));
    console.log(
      `✅ ${newArticles.length} novas notícias de entretenimento adicionadas`
    );
    console.log(`📊 Total de entretenimento: ${currentData.length}`);
  } catch (error) {
    console.error("❌ Erro ao atualizar entretenimento:", error.message);
    process.exit(1);
  }
}
