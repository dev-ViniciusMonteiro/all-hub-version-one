...
function transformToFormat(articles) {
  return articles.map((article) => ({
    ...
    image: article.image,
    category: "curiosidades-tendencias",
...
async function updateUtilidades() {
  try {
    console.log("🔄 Fazendo scraping de utilidades...");
    const [canal] = await Promise.allSettled([scrapeCanaltech()]);
    ...
    console.log(
      `   Canaltech: ${
        canal.status === "fulfilled"
          ? "✅ OK (" + canal.value.length + " artigos)"
          : "❌ FALHOU - " + canal.reason?.message
      }`
    );

    const allArticles = [...(canal.status === "fulfilled" ? canal.value : [])];
    if (allArticles.length === 0) {
      console.log("⚠️ Nenhuma notícia de utilidades encontrada");
      return;
    }
    console.log(
    ...
    const filePath = path.join(__dirname, "../src/data/json/utilidades.json");
    let currentData = [];
    try {
    ...
    if (newArticles.length === 0) {
      console.log("ℹ️ Nenhuma notícia nova de utilidades encontrada");
      return;
    }
    currentData = [...newArticles, ...currentData].slice(0, 50);
    await fs.writeFile(filePath, JSON.stringify(currentData, null, 2));
    console.log(
      `✅ ${newArticles.length} novas notícias de utilidades adicionadas`
    );
    console.log(`📊 Total de utilidades: ${currentData.length}`);
  } catch (error) {
    console.error("❌ Erro ao atualizar utilidades:", error.message);
    process.exit(1);
  }
}
